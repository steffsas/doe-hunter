...
func TestDoQQuery_RealWorld(t *testing.T) {
	...
	t.Run("try multiple sequential valid quic queries", func(t *testing.T) {
		t.Parallel()
		qm := new(dns.Msg)
		qm.SetQuestion(doqNameQuery, dns.TypeA)
...
func TestDoQQuery_SkipCertificateVerify(t *testing.T) {
	...
	t.Run("do not skip certificate verify", func(t *testing.T) {
		...
		dialHandler.On("Query", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, certError)
...
func (m *mockedDialHandler) Query(ctx context.Context, addr net.Addr, tlsConf *tls.Config, conf *quic.Config) (query.QuicConn, error) {
